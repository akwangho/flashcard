...
- **特殊情況**: 若單字為負數（非常熟），按 S 鍵後會 +1（例如 -5 → -4, -1 → 0）
- **暫時刪除狀態下按 S**: 若單字正處於暫時刪除狀態（pendingRemoval），按 S 鍵會直接標記為非常熟（-999），方便遙控器操作
- **視覺回饋**: 進度區域顯示 `★N`（N 為不熟程度數字），顏色根據等級變化。UI 上負數一律顯示為 0（避免使用者看到 -1, -2 等負數），但後端 Google Sheet 仍儲存實際負數值。顏色方面，只有 -999（非常熟）顯示綠色，-1~-998 與 0 相同顯示灰色
...
- **即時預覽**: 點擊單字卡標記為待刪除時，不熟程度數字立即在 UI 上顯示減 1 後的值（尚未寫入後端），提升使用者體驗；若使用者取消刪除（按 R 或點擊恢復按鈕），則恢復為原本的數字。預覽數字同樣遵循負數不顯示規則，最小顯示為 0
...
- **疊加**: 可與不熟程度篩選、複習時間篩選、要會拼篩選同時使用，結果取交集
...
| `M` 鍵 | 切換靜音/取消靜音 |
| `F` 鍵 | 切換全螢幕 |
| `S` 鍵 / F5 | 增加不熟程度 +1（任何負數 +1, 0 → 1, ..., 9 → 10）；暫時刪除狀態下按 S 直接標記為 -999 |
...
- **選單結構**:
  - **⭐ 篩選**（可展開/收合的母選單，點擊展開四個子選單）
    1. ⭐ 不熟程度篩選
    2. 📅 複習時間篩選
    3. ✍️ 要會拼篩選
    4. 📝 類型篩選（多選：單字/片語/句子）
  - 📖 快速複習（依優先度推薦複習單字，顯示待複習徽章）
  - **🎯 測驗**（可展開/收合的母選單，點擊展開兩個子選單）
    4. 🎯 快速測驗 (10 題)
    5. 📝 完整測驗
  - **工具**
    6. ✏️ 編輯當前單字
    ...
    8. 🔄 重新載入單字
  - **快速模式**
    9. 🎧 用聽的背單字（中文先出現、字母發音開、中文發音開、9秒、智慧計時開）
    10. 👂 用聽的認識單字（英文先出現、字母發音關、中文發音開、4.5秒、智慧計時開、延遲發音關）
    11. 📝 日常複習（混合出現、延遲發音開、字母發音關、中文發音關、智慧計時開）
  - **設定**
    12. 📋 單字檔設定
    13. 🔊 語音設定
    14. ⚙️ 一般設定
    15. 📺 全螢幕
- **母選單行為**: 「篩選」與「測驗」為可展開的母選單項目，點擊後展開顯示子選單，右側箭頭 ▸ 會旋轉為 ▾ 表示展開狀態；同一時間只能展開一組子選單，展開另一組時前一組自動收合；關閉選單時所有子選單自動收合
...
- **描述**: 選單內「快速模式」區塊提供 3 種預設模式，一鍵套用多項設定，免去逐一切換
- **模式列表**:
...
- **行為**: 點擊後立即套用設定、儲存到 localStorage、關閉選單、顯示通知、重新顯示當前單字（歸零計時、重新判斷顯示順序，避免問題與答案語言相同）
...
| 單字 | B 欄 | 英文單字（文字輸入框） |
| 翻譯 | C 欄 | 中文翻譯（文字輸入框） |
| 不熟程度 | D 欄 | -1~10 滑桿 + -999~10 數字輸入框（雙向同步），-999 顯示「非常熟 (-999)」（綠色），-1~-998 及 0~10 顯示 ★N 及顏色變化 |
| 要會拼 | A 欄 | 開關 |
...
- **模態框內容**:
  - 總是顯示可複習的單字（不會出現「沒有單字」的情況）
  - 有到期單字時顯示「共 N 個單字，其中 M 個建議優先複習」
  - 無到期單字時顯示「共 N 個單字，依複習優先度排列」
  - 數量選擇：10、20、30、50、100 或「全部 (N)」的按鈕選項
- **優先度排序**: 系統依以下優先度排列複習單字：
  1. 已到期/逾期的單字（越逾期越優先，SRS Box 等級越低越優先）
  2. 從未進入 SRS 系統的單字（不熟程度越高越優先）
  3. 尚未到期但最接近複習日的單字
...
| `difficulty-display` | span | 不熟程度顯示區（★ + 數字，負數顯示為 0） |
