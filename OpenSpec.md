...
```javascript
{
  ...
  difficultyLevel: Number,   // 不熟程度 0-10（來源：D 欄 '*' 的數量，0=已熟悉，10=最不熟）
...
| 欄位 | 標題列 (第 1 列) | 資料列 (第 2 列起) | 必填 |
|------|------|------|------|
| A 欄 (第 1 欄) | `總數` | A2 = 有效單字數量（數字） | 否 |
| B 欄 (第 2 欄) | `單字` | 英文單字 | 是 |
| C 欄 (第 3 欄) | `翻譯` | 中文翻譯 | 是 |
| D 欄 (第 4 欄) | `不熟程度` | 1-10 個 `*` 符號（`*`=1, `**`=2, ..., `**********`=10，空=0） | 否 |
...
#### 4.2.1 多層級不熟程度系統
- **描述**: 每個單字有 0-10 的不熟程度等級，以 D 欄的 `*` 符號數量表示
- **等級意義**:
  - `0`（無 `*`）= 已熟悉
  - `1-2`（★★）= 稍不熟，顯示黃色
  - `3-4`（★★★★）= 中等不熟，顯示橘色
...
- **同步到後端**: 透過 `google.script.run.updateWordDifficulty()` 即時寫回 Google Sheet D 欄

#### 4.2.3 減少不熟程度（自動）
- **描述**: 每次單字被暫時移除（點擊單字卡）時，自動減少不熟程度 -1（最低 0）
- **設計理念**: 使用者覺得已熟悉的單字，移除時自動降低不熟程度，模擬間隔重複學習

#### 4.2.4 不熟程度篩選
- **描述**: 透過模態框篩選特定不熟程度以上的單字
- **篩選選項**: 全部 / ★1 以上 / ★3 以上 / ★5 以上 / ★7 以上 / ★10（最不熟）
- **行為**: 篩選後重新洗牌並從頭開始；若無符合條件的單字，彈出提示並重設篩選
- **疊加**: 可與複習時間篩選同時使用，結果取交集
...
- **匯出資料欄位**: 總數（A 欄）、單字（B 欄）、翻譯（C 欄）、不熟程度（D 欄，`*` 符號）、圖片 URL（E 欄）
- **標題列**: 匯出時自動在第 1 列寫入標題
...
| `countValidWords(sheet)` | 讀取工作表 A2 格的值作為有效單字數 |
...
- [ ] **多層級不熟程度系統**: 0-10 級不熟程度、★ 星號顯示、篩選模態框、點擊增加/移除減少、同步到後端
