...
- **智慧計時**: 可選功能（預設關閉），啟用後顯示中文翻譯的第二階段（Phase 2）等待時間會根據中文文字長度自動縮短。演算法：`smartDelay = max(1.2, 1.0 + charCount * 0.15)` 秒，上限為原始 `delayTime`。僅在中文為第二語言時生效（`english-first` 或 `mixed` 模式中先英後中），反向模式不適用。常數定義於 `APP_CONSTANTS`：`SMART_TIMER_BASE_TIME`（1.0 秒）、`SMART_TIMER_PER_CHAR_TIME`（0.15 秒/字元）、`SMART_TIMER_MIN_TIME`（1.2 秒）
...
- **描述**: 使用者按 D 鍵可將當前單字標記為「非常熟」（-1），需要按兩次確認，確認後進入類似暫時刪除的狀態
- **行為流程**:
  1. 第一次按 D：暫停計時器和進度條，顯示提示「再按一下 D 設為非常熟（ESC 取消）」
  2. 第二次按 D（3 秒內）：進入暫時刪除狀態，行為類似點擊單字卡（4.1.3）
     ...
     - 出現「恢復單字」按鈕
     - 顯示「✓ 已設為非常熟」提示
  3. 等待延遲時間後 → 正式標記為非常熟（-1）、同步到後端、從此輪單字中移除、跳到下一個
  4. 若在等待期間使用者點擊「恢復單字」或按左方向鍵 → 取消標記，恢復原始不熟程度，回到正常播放
  5. 按 ESC 或 3 秒超時（僅限第一次按 D 後尚未進入暫時刪除狀態時）：取消操作，恢復計時器和進度條
- **已在暫時刪除狀態中按 D**:
  - 若單字已因點擊進入暫時刪除狀態（`pendingRemoval`），按兩次 D 可將其「升級」為非常熟標記，保持暫時刪除狀態不變，延遲時間到期後以 -1 處理
- **-1 篩選模式**: 直接設定為 -1 並更新顯示（不進入暫時刪除狀態）
...
- **智慧計時**: 開關（預設關閉）。啟用後，顯示中文翻譯後會根據文字長度自動縮短等待時間，加速切換節奏。進度條動畫也會同步使用縮短後的時長
...
- **優先度排序**: 系統依以下優先度排列複習單字：
  ...
  3. 尚未到期但最接近複習日的單字
- **複習流程**: 選擇數量後點擊「開始複習」，系統依優先度取前 N 個單字，使用一般閃卡模式進行複習
