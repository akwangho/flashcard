...
- **單元測試**: 使用 Jest + jsdom，執行 `npx jest` 可運行 295 個測試案例（涵蓋語音等待機制、導覽、不熟程度、暫停/繼續、SRS 間隔重複、單字檔歷史等核心功能）
...
- **語音等待機制**: 當字母拼讀或中文語音功能啟用時，若延遲時間到期而語音尚未播放完畢，系統會自動等待語音播放完成後才進行切換（最長等待 60 秒）。此機制確保長單字的字母拼讀和長翻譯的中文朗讀不會被截斷。使用者手動操作（點擊「下一個」、「上一個」等）不受此機制影響，會立即切換並取消語音
