...
- **確認暫刪後還原**: 延遲結束並確認暫時刪除後，若已切到下一張單字，可再按 **↑／←**（或「上一個」按鈕）還原**上一筆**已確認的暫刪（`removalUndoStack`，LIFO），並還原該字的不熟程度、複習標記、SRS 與當輪順序；與僅在待刪除等待期內按鍵取消（`cancelRemoval`）不同

#### 4.1.5 上一個/下一個導覽
- **描述**: 使用者可以手動切換到上一個或下一個單字
- **上一個**（優先順序）: (1) 若單字正處於待暫刪狀態 → 取消移除；(2) 若有已確認暫刪的還原紀錄 → 還原上一筆確認暫刪；(3) 否則從導覽歷史堆疊取出前一個狀態（包含索引與該步驟的單字序列）
